...
import { getTableUsers } from "@/server/server-functions/user-functions";
import { columns } from "./user-columns";

export function UserTable() {
  const getTable = useServerFn(getTableUsers);

  const { table, query } = useQueryTable({
    queryOptions: (params) => ({ queryKey: ["users", params], queryFn: () => getTable({ data: params }) }),
